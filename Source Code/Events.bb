...
Function InitEvents()
	...
	CreateEvent("spawndclass","room3_2",0,0.6+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room2",0,0.6+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room3z2",0,0.7+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room1123",0,0.8+(0.2*SelectedDifficulty\aggressiveNPCs))
	;CreateEvent("spawndclass","room2pipes",0,0.4+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room2pit",0,0.5+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room3tunnel",0,0.6+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room2toilets",0,0.7+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room2z3",0,0.6+(0.2*SelectedDifficulty\aggressiveNPCs))
	CreateEvent("spawndclass","room2poffices",0,0.4+(0.2*SelectedDifficulty\aggressiveNPCs))
	...
	;096 spawns in the first (and last) lockroom2 - fuck this shit
				...
				If e\EventState = 0 Then
					If PlayerRoom = e\room Then
						...
						If e\room\NPC[0]=Null
							...
							e\room\NPC[6] = CreateNPC(NPCtypeD, EntityX(e\room\Objects[8], True), EntityY(e\room\Objects[8], True), EntityZ(e\room\Objects[8], True))
							SetNPCFrame(e\room\NPC[6], 381) : e\room\NPC[6]\State = 3
							PointEntity(e\room\NPC[6]\Collider, Collider)
							Local d9341tex = LoadTexture_Strict("GFX\npcs\classd3.jpg")
							EntityTexture e\room\NPC[6]\obj, d9341tex
							FreeTexture d9341tex
							...
							If e\EventState > 900 And e\room\RoomDoors[5]\open Then
								...
								Else
									If e\EventState-FPSfactor < 900+4*70 Then 
										PlaySound_Strict(IntroSFX(11)) : LightBlink = 3.0 ;first lightblink
										PointEntity(e\room\NPC[6]\Collider, e\room\NPC[1]\Collider) ; makes d-9341 look at franklin and ulgrin
										
										PlaySound2 (StoneDragSFX, Camera, Curr173\Collider)
										;PointEntity Curr173\Collider, e\room\NPC[2]\Collider
										RotateEntity (Collider, 0,150,0)
										RotateEntity HeadModel, 0,-60,0
										MoveEntity (Collider, 0,0,1)
										;PositionEntity BodyModel, EntityX(Collider), EntityY(Collider), EntityZ(Collider)
										MoveEntity (BodyModel, 0,0,1)
										
										;PositionEntity Collider, e\room\x-96*RoomScale, 0.31, e\room\z+592*RoomScale, True
										;RotateEntity BodyModel,0,190,0
									...
									If e\room\NPC[2]\State <> 1 And KillTimer >= 0
										If EntityZ(e\room\NPC[2]\Collider) < e\room\z-1150*RoomScale Then
											...
											LightBlink = 6.0 ;second lightblink
											...
											If (Not alreadydone) Then
												Select Rand(2)
													Case 1
														GotoVentLocation("room2closets")
														playclosetevent% = True
													Case 2
														GotoVentLocation("room2testroom2")
														breaktestroom2glass% = True ; allows to playing to breach the glass in room2testroom2
												End Select
												alreadydone = True
											EndIf
							...
							Else
								...
								If e\room\NPC[2]\State <> 1
									...
									If e\room\RoomDoors[5]\openstate = 0 Then 
										...
										If e\room\NPC[4] <> Null Then RemoveNPC(e\room\NPC[4])
					...
					If e\EventState2 = 1 Then ;in the second room
								...
								For i = 9 To 10
									...
									If dist<6.0 Then 
										If dist<100.0*RoomScale Then
											...
											If KillTimer = 0 Then
												DeathMSG = "The remains of SCP-173 were discovered at [REDACTED]. Subject's body appears to be severely decomposed - Most likely due to SCP-106."
						...
						If EntityY(Collider) < -1600*RoomScale Then
							...
							Else ;the player is not at the exit, must've fallen down
								...
								If KillTimer => 0 Then 
									PlaySound_Strict HorrorSFX(8)
									DeathMSG = "The remains of SCP-173 were discovered in [REDACTED]. Subject's body appears to be severely decomposed - Most likely due to SCP-106."
								EndIf
				...
				If playclosetevent% = True Then
					...
					Else
						...
						Else
							...
							EndIf
						EndIf
					;HideEntity e\room\RoomVents[1]\obj%
					EndIf
				EndIf
				;[End Block]
			Case "room2doors173"
								...
								If KillTimer => 0 Then 
									For i = 0 To 2
										If Distance(EntityX(Collider),EntityX(e\room\Objects[i],True),EntityZ(Collider),EntityZ(e\room\Objects[i],True)) < 250.0*RoomScale Then
											...
											LightFlash = 0.4
											CameraShake = 1.0
											DeathMSG = "hey there player, report this death message if you get it. Thanks."
										EndIf
									Next
								EndIf
                ...
                If PlayerRoom = e\room Then
                    Select e\EventState
                        Case 0
							...
							PlaySound2(LoadTempSound("SFX\Horror\Horror7.ogg"), Camera, e\room\RoomDoors[1]\frameobj)
							PlaySound2(LoadTempSound("SFX\Door\DoorError.ogg"), Camera, e\room\RoomDoors[2]\frameobj)
                        Case 2
                            e\EventState = 2
                            RemoveEvent (e)
                    End Select
...
;					If e\EventState = 0 Then
;						CreateBreakableObject(r\x - 632.0 * RoomScale, 224.0*RoomScale, r\z - 208.0 * RoomScale, 0, 0, 0, r\x - 669.0 * RoomScale, 0.5, r\z - 16.0 * RoomScale, "glass", r)
;						e\EventState = 1
;					EndIf
;				EndIf
				;[End Block]
			...
			Case "914"
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState > 0 Then
						...
						If EntityDistance(Collider, e\room\Objects[2]) < 1 Then;Distance(EntityX(Collider), EntityX(e\room\Objects[2], True),EntityZ(Collider), EntityZ(e\room\Objects[2], True)) < (170.0 * RoomScale) Then
