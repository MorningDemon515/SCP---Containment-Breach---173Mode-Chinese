...
; 173 Mod NPCs
Const NPCtypeD2% = 23, NPCtypeMTF2% = 24
...
Type NPCs
	...
	Field PreppedTransportCell% = False
	Field IsDead%
	Field IsLooking%
...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeMTF,NPCtypeMTF2
			...
			n\killindicator = CreateSprite()
			Local tex% = LoadTexture_Strict("GFX\interactneck.png",1+2)
			ScaleSprite n\killindicator,0.04,0.04
			EntityOrder n\killindicator,-1
			EntityTexture n\killindicator,tex
			EntityFX n\killindicator,1+8
			HideEntity n\killindicator
		...
		Case NPCtypeD,NPCtypeD2
			...
			n\obj2 = CreateSprite() ; Blink Sprite
			EntityBlend (n\obj2, 3)
			SpriteViewMode(n\obj2, 3)
			ScaleSprite n\obj2,0.08*(30.0/31.0),0.08
			HideEntity n\obj2
			
			If n\NPCtype = NPCtypeD2 Then ; Cosmetics and random texture if NPCtypeD2
				
				n\killindicator = CreateSprite()
				Local texef% = LoadTexture_Strict("GFX\interactneck.png",1+2)
				ScaleSprite n\killindicator,0.04,0.04
				EntityOrder n\killindicator,-1
				EntityTexture n\killindicator,texef
				EntityFX n\killindicator,1+8
				HideEntity n\killindicator
				...
				If random <= 9 Then
					dtex = LoadTexture_Strict("GFX\npcs\d2\classd" + random + ".jpg")
				Else ; if 10 then
					dtex = LoadTexture_Strict("GFX\npcs\d2\classd3.jpg") ;d9341
				EndIf
				EntityTexture n\obj, dtex
				FreeTexture dtex
				
				Local facebone% = FindChild(n\obj,"Bip01_Head")
				...
				Select Rand(1,8)
					Case 2
						item% = LoadMesh("GFX\npcs\d2\NVG.b3d")
						identifier$ = "nvg"
						EntityParent(item, facebone)
						PositionEntity item,-3.4,0,6.5
						ScaleEntity item, 1.0,1.0,1.0
					Case 6
						item% = LoadMesh("GFX\npcs\d2\gasmask.b3d")
						identifier$ = "gasmask"
						EntityParent(item, facebone)
						PositionEntity item,-3.4,0,3.1
						ScaleEntity item, 1.0,1.0,1.0
					Default
				End Select
			EndIf
		...
		Case NPCtype049
			...
			n\killindicator = CreateSprite()
			Local texe34f% = LoadTexture_Strict("GFX\interactneck.png",1+2)
			ScaleSprite n\killindicator,0.04,0.04
			EntityOrder n\killindicator,-1
			EntityTexture n\killindicator,texe34f
			EntityFX n\killindicator,1+8
			HideEntity n\killindicator
...
Function UpdateNPCs()
	...
	Local i%, dist#, dist2#, angle#, x#, y#, z#, prevFrame#, PlayerSeeAble%, RN$, AbleToSeePlayer%, temp2%
	...
	For n.NPCs = Each NPCs
		...
		Select n\NPCtype
			...
			Case NPCtype049
				...
				Local neckbone049% = FindChild(n\obj,"Bone_012")
				PositionEntity n\killindicator, EntityX(neckbone049, True), EntityY(neckbone049, True)+0.05, EntityZ(neckbone049, True)
				
				EntityColor n\killindicator, 255-(Float(Sin(MilliSecs()/4.0))*150),0,0
				
				; Eye contact handler
				If (Not n\IsLooking) Then
					Looking = False
				Else
					Looking = True
				EndIf
				
				n\BlinkTimer = n\BlinkTimer - FPSFactor ; Start NPC Blinktimer
				If n\BlinkTimer < -70*1 Then
					n\BlinkTimer = 70.0*Rnd(10.0,15.0)
				EndIf
				
				PlayerSeeAble% = MeNPCSeesPlayer(n)
				
				If PlayerSeeAble%=True And n\BlinkTimer > 0.0 And Contained% <> True And dist < 5.5 And n\State <> 3 Then ; If they are still remaining eye contact (Not blinking)
					n\IsLooking = True
					n\State = 6
					HideEntity n\killindicator
				Else ; If they blink
					If dist < 0.5 And (Not n\IsLooking) And (Not scp173j%) And n\State <> 3 Then
						If MouseHit1 Then
							n\State = 3
							PlaySound_Strict(NeckSnapSFX(Rand(0,2)))
							PlaySound_Strict(DamageSFX(0))
							CameraShake = 1.5
							n\PathStatus = 0 ; Remove path
							n\CurrSpeed = CurveValue(0.0, n\CurrSpeed, 5.0) ; Set speed to 0
						EndIf
					EndIf
					ShowEntity n\killindicator
					If dist > 8 Then
						HideEntity n\killindicator
					Else
						n\IsLooking = False
				...
				Else
					...
					Select n\State
						...
						Case 2 ;being active
							;[Block]
							If (dist < HideDistance*2) And (Not n\Idle) And PlayerInReachableRoom(True) And n\State <> 3 Then
								n\SoundChn = LoopSound2(n\Sound, n\SoundChn, Camera, n\Collider)
								PlayerSeeAble% = MeNPCSeesPlayer(n)
								If PlayerSeeAble%=True Lor n\State2>0 Lor Looking = True Then ;Player is visible for 049's sight - attacking
						...
						Case 3 ;The player was killed by SCP-049
							;[Block]
							n\State2 = 70*5
							AnimateNPC(n, 537, 660, 0.4, False)
							HideEntity n\killindicator
							
							If n\State2 < -70*1 Then
								n\State = 1
							EndIf
						...
						Case 6
							...
							If (Not n\IsLooking) Then ; If they aren't looking at SCP-173
								;n\State = 0
								AnimateNPC(n, 1324, 1548, 0.4, True)
								If dist > 0.6 Then n\State = 1
								If (Not Contained%) Then ShowEntity n\killindicator
							Else ; If they are looking
								PointEntity n\obj,Collider
								RotateEntity n\Collider,0,CurveAngle(EntityYaw(n\obj),EntityYaw(n\Collider),10.0),0
								AnimateNPC(n, 269, 346, n\CurrSpeed*43)
							EndIf
							;[End Block]
					End Select
				EndIf
			...
			Case NPCtypeMTF ;------------------------------------------------------------------------------------------------------------------
				...
				;[End Block]
			Case NPCtypeMTF2 ;MTF2
				;[Block]
				dist = EntityDistance(Collider, n\Collider) ; Distance from player to NPC
				prevFrame = AnimTime(n\obj)
				
				If (Not n\IsDead) Then
					
					Local neckbone2% = FindChild(n\obj,"neck")
					PositionEntity n\killindicator, EntityX(neckbone2, True), EntityY(neckbone2, True)+0.05, EntityZ(neckbone2, True)
					...
					n\SoundChn = LoopSound2(MTFSFX(6),n\SoundChn,Camera,n\Collider)
					...
					If n\BlinkTimer < -70*1 Then
						n\BlinkTimer = 70.0*Rnd(10.0,15.0)
					ElseIf n\BlinkTimer > -70*1 And n\BlinkTimer < 0.0 Then
					EndIf
					
					PlayerSeeAble% = MeNPCSeesPlayer(n)
					
					If PlayerSeeAble%=True And n\BlinkTimer > 0.0 And Contained% <> True Then ; If they are still remaining eye contact (Not blinking)
						n\IsLooking = True
						n\TargetEnt = Collider
						n\State = 2
						If ChannelPlaying(n\SoundChn2)=False
							If n\MTFLeader <> Null Then
							Else
								If n\PrevState<>3 Then
									If n\SoundChn2 <> 0 Then StopChannel(n\SoundChn2)
									n\SoundChn2 = PlaySound_Strict(LoadTempSound("SFX\Character\MTF\173\newlines\173contact"+Rand(1,6)+".ogg"))
									n\PrevState=3
								EndIf
							EndIf
						EndIf
					Else ; If they blink
						n\IsLooking = False
						If dist < 0.5 And (Not n\IsLooking) And Contained% = False And (Not scp173j%) Then
							If MouseHit1 Then
								n\IsDead = True ; Kill the NPC
								PlaySound_Strict(NeckSnapSFX(Rand(0,2)))
								PlaySound_Strict(DamageSFX(0))
								If corrosive Then
									PlaySound_Strict(OldManSFX(Rand(3,5)))
									PlaySound_Strict(OldManSFX(Rand(0,2)))
									If Not PlayerRoom\RoomTemplate\DisableDecals Then
										de.Decals = CreateDecal(0, EntityX(n\Collider), 0.01, EntityZ(Collider), 90, Rand(360), 0)
										de\Size = 0.05 : de\SizeChange = 0.01 : EntityAlpha(de\obj, 0.8) : UpdateDecals
									EndIf
									fall# = (0.014 * 5) / (1.0+CrouchState)
									MoveEntity n\Collider, 0,-fall/8*FPSFactor,0
								EndIf
								CameraShake = 1.5
								n\HP = 0
								n\PathStatus = 0 ; Remove path
								n\CurrSpeed = CurveValue(0.0, n\CurrSpeed, 5.0) ; Set npc speed to 0
							EndIf
						EndIf
					EndIf
					
					If Contained% = True And n\IsDead <> True Then
						If n\MTFLeader = Null Then;And containtimer > 0.0 Then
							Local lookpvt% = CreatePivot() ; im sorry you must see this
							PointEntity followpvt, n\Collider
							dist2 = EntityDistance(followpvt, n\Collider)
							MoveEntity followpvt, 0, 0, 0.016*FPSfactor*Max(Min((dist2*2-1.0)*0.5,1.0),-0.5)
							PositionEntity Collider, EntityX(followpvt), EntityY(followpvt), EntityZ(followpvt)
							ResetEntity(Collider)
							PositionEntity lookpvt, EntityX(n\Collider), EntityY(n\Collider) + 0.3, EntityZ(n\Collider)
							FreeEntity(lookpvt)
						EndIf
					EndIf
					...
					Select n\State
						Case 0 ; idle
							n\CurrSpeed = CurveValue(0.0, n\CurrSpeed, 5.0)
							Animate2(n\obj, AnimTime(n\obj), 79, 311, 0.5)
							n\State = 1
						Case 1 ; wandering around
							If n\PathStatus = 1
								If n\Idle <= 0.0 Then
									If n\Path[n\PathLocation]=Null Then 
										If n\PathLocation > 19 Then 
									...
									Else
										...
										If n\MTFLeader <> Null Then
											n\CurrSpeed = CurveValue(n\Speed*1.2, n\CurrSpeed, 20.0)
										Else
											n\CurrSpeed = CurveValue(n\Speed*0.7, n\CurrSpeed, 20.0)
										EndIf
							...
							If n\PathTimer# <= 0.0
								If (Not PlayerSeeAble%) Lor Contained% = True Then
									...
									Else
										For r = Each Rooms
											If ((Abs(r\x-EntityX(n\Collider,True))>24.0) Lor (Abs(r\z-EntityZ(n\Collider,True))>24.0)) And (Rand(1,Max(4-Int(Abs(r\z-EntityZ(n\Collider,True)/8.0)),2))=1) Then
												x = r\x
												y = 0.1
												z = r\z
										...
										For e.Events = Each Events
											If e.Events <> Null And e\EventName = "room2tesla" Then
												If Abs(EntityX(n\Collider)-EntityX(e\room\obj,True))<4.0 And Abs(EntityZ(n\Collider)-EntityZ(e\room\obj,True))<4.0 Then
													If e\EventState2 <= 70*3.5 Then
														...
														n\Idle = 70*10
														If dist < 5 Then
															n\SoundChn2 = PlaySound2(LoadTempSound("SFX\Character\MTF\173\newlines\tesla_disable"+Rand(1,2)+".ogg"), Camera, n\Collider, 20)
														EndIf
						...
						Case 2 ; SCP-173 spotted!
							...
							If (Not n\IsLooking) Then ; If they aren't looking at SCP-173
								...
								If (Not Contained%) Then ShowEntity n\killindicator
							Else ; If they are looking
								Animate2(n\obj, AnimTime(n\obj), 79, 194, 0.5) ; Alert Idle
								PointEntity n\obj, Collider
								RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\obj), EntityYaw(n\Collider), 10.0), 0
								HideEntity n\killindicator
							EndIf
						Case 3 ; open slot
						Case 4 ; glancing at 173
							n\PathStatus = 0
							n\CurrSpeed = CurveValue(0.0, n\CurrSpeed, 5.0)
							Animate2(n\obj, AnimTime(n\obj), 962, 1258, 0.5)
							PointEntity n\obj, Collider
							RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\obj), EntityYaw(n\Collider), 10.0), 0
						Case 5 ; shooting a target
							If n\Target <> Null And n\Target\HP > 0 Then
								If EntityVisible(n\Collider, n\Target\Collider) Then
									...
									Animate2(n\obj, AnimTime(n\obj), 79, 311, 0.5)
									DebugLog "Seein the target, reload timer is: "+n\Reload
									...
									If n\Reload = 0 Then
										...
										If n\Target\HP = 0 Then
											...
											If Not alreadydone
												n\SoundChn2 = PlaySound_Strict(LoadTempSound("SFX\Character\MTF\173\newlines\classd_terminated"+Rand(3,13)+".ogg")) ; target down
												alreadydone = True
											EndIf
								...
								EndIf
							EndIf
						Case 6 ;following another team member
							If n\Target <> Null Then
								n\Reload = 200
								...
								n\CurrSpeed = CurveValue(n\Speed*Min(1.0,(dist2-1.0)*3), n\CurrSpeed, 20.0)
								MoveEntity n\Collider, 0, 0, n\CurrSpeed * FPSfactor
								Animate2(n\obj, AnimTime(n\obj), 488, 522, n\CurrSpeed * 16)
								...
								If dist2 > 7.0 Then
									If n\PathTimer =< 0 Then
										...
										If n\PathStatus = 1 Then n\State = 1
									Else
										n\PathTimer = Max(n\PathTimer-FPSfactor,0)
									EndIf
								EndIf
...
;							EndIf
					...
					For n2.NPCs = Each NPCs
						If (n2<>n) And n2\IsDead <> True Then
					...
					If (Not n\IsLooking) Then
						For n2.NPCs = Each NPCs
							If n2\NPCtype = NPCtypeD2
								If EntityDistance(n\Collider,n2\Collider)<8.0 And n2\IsDead = False
									n\PathTimer = 0.0
									n\PathStatus = 0
									;n\Reload = 70*3
									n\Target = n2
				...
				Else
					HideEntity n\killindicator
					Animate2(n\obj, AnimTime(n\obj), 1525, 1547, 0.5, False)
				EndIf
				
				RotateEntity n\obj, EntityPitch(n\Collider)-90.0, EntityYaw(n\Collider), 0
				PositionEntity n\obj,EntityX(n\Collider,True),EntityY(n\Collider,True)-0.15,EntityZ(n\Collider,True),True
				
				If n\CurrSpeed > 0.01 Then
					If prevFrame = 500
						PlaySound2(StepSFX(2,0,Rand(0,2)),Camera, n\Collider, 8.0, Rnd(0.5,0.7))
					ElseIf prevFrame = 505
						PlaySound2(StepSFX(2,0,Rand(0,2)),Camera, n\Collider, 8.0, Rnd(0.5,0.7))
					EndIf
				EndIf
				
				n\Idle = Max(0.0, n\Idle-FPSfactor)
				;[End Block]
...
;								EndIf
			...
			Case NPCtypeD2 ;D2
				...
				RotateEntity(n\Collider, 0, EntityYaw(n\Collider), EntityRoll(n\Collider), True)
				prevFrame = AnimTime(n\obj)
				Local b% = FindChild(n\obj,"Bip01_Head")
				EntityParent n\obj2, b
				PositionEntity(n\obj2, -15,0,0)
				MoveEntity(n\obj2, 0, 10.6 * 0.11, 0 * 0.11)
				
				If Not (n\IsDead) Then
					;[Block] Blinking Code
					
					Local neckbone% = FindChild(n\obj,"Bip01_Neck")
					PositionEntity n\killindicator, EntityX(neckbone, True), EntityY(neckbone, True)+0.05, EntityZ(neckbone, True)
					...
					n\BlinkTimer = n\BlinkTimer - FPSFactor ; Start NPC Blinktimer
					If n\BlinkTimer < -70*5 Then
						If (Not inroomwithgas) Then
							n\BlinkTimer = 70.0*Rnd(10.0,15.0)
						Else
							n\BlinkTimer = 70.0*Rnd(4.0,6.0)
						EndIf
					EndIf
					
					PlayerSeeAble% = MeNPCSeesPlayer(n)
					
					If PlayerSeeAble%=True And n\BlinkTimer > 0.0 And Contained% <> True And dist < 5.5 Then ; If they are still remaining eye contact (Not blinking)
						n\IsLooking = True
						n\State = 2
					Else ; If they blink
						If dist < 0.5 And (Not n\IsLooking) And (Not scp173j%) Then
							If MouseHit1 Then
								n\IsDead = True ; Kill the NPC
								PlaySound_Strict(NeckSnapSFX(Rand(0,2)))
								PlaySound_Strict(DamageSFX(0))
					...
					Select n\State
						Case 0 ; Idle
							If Not (n\IsDead) Then AnimateNPC(n, 1164, 1268, 0.4, True) ; Calm idle
							
							If Rnd(4)=1 Then
								n\State = 1
							EndIf
						Case 1 ; Active - Moving around the facility
							...
							If n\PathStatus = 1
								...
								Else
									...
									If Not (n\IsDead) Then AnimateNPC(n, 236, 260, 0.4, True) ; Alert walk
									n\CurrSpeed = CurveValue(0.015, n\CurrSpeed/1.5, 5.0)
								...
								For e.Events = Each Events
									If e.Events <> Null And e\EventName = "room2pit" Then
										If Abs(EntityX(n\Collider)-EntityX(e\room\obj,True))<4.0 And Abs(EntityZ(n\Collider)-EntityZ(e\room\obj,True))<4.0 Then
											n\BlinkTimer = 70.0*Rnd(5.0,7.0)
										EndIf
									EndIf
								Next
							Else
								If n\State = 1 Or n\State = 0 Then
									n\CurrSpeed = CurveValue(0.0, n\CurrSpeed, 5.0)
									If Not (n\IsDead) Then AnimateNPC(n, 1164, 1268, 0.4, True)
								Else
									If Not (n\IsDead) Then AnimateNPC(n, 1324, 1548, 0.4, True)
								EndIf
							EndIf
							
							If n\PathTimer# = 0.0
								If (Not PlayerSeeAble%) Then
									For r = Each Rooms
										If ((Abs(r\x-EntityX(n\Collider,True))>24.0) Lor (Abs(r\z-EntityZ(n\Collider,True))>24.0)) And (Rand(1,Max(4-Int(Abs(r\z-EntityZ(n\Collider,True)/8.0)),2))=1) Then
											x = r\x
											y = 0.1
						...
						Case 2 ; Encountering SCP-173
							...
							If (Not n\IsLooking) Then ; If they aren't looking at SCP-173
								...
								If (Not Contained%) Then ShowEntity n\killindicator
							Else ; If they are looking
								If Not (n\IsDead) Then AnimateNPC(n, 1324, 1548, 0.4, True) ; Alert Idle
								PointEntity n\obj, Collider
								RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\obj), EntityYaw(n\Collider), 10.0), 0
								HideEntity n\killindicator
							EndIf
						Case 3
							;Animate2(n\obj, AnimTime(n\obj), 260, 236, 0.4, False)
							n\State = 1
					End Select
					;[End Block]
				Else ; If the NPC is dead
					Local camrot% = (EntityYaw(Camera) - EntityYaw(n\obj) + 45 + 180) Mod 360
					n\State2 = 0.0
					If camrot > 90 Then
						n\State2 = 1.0
						If camrot > 180 Then
					...
					HideEntity n\killindicator
					;Animate2(n\obj, AnimTime(n\obj), 678, 711, 0.5, False)
					n\CurrSpeed = 0
					n\IsLooking = False
					
					Select n\State2
						Case 0.0
							Animate2(n\obj, AnimTime(n\obj), 678, 711, 0.5, False) ; front
						Case 1.0
							Animate2(n\obj, AnimTime(n\obj), 630, 677, 0.5, False) ; left
						Case 2.0
							Animate2(n\obj, AnimTime(n\obj), 555, 629, 0.5, False) ; back
					End Select
				EndIf
				
				;[Block] Walking Sounds
				If n\CurrSpeed > 0.01 Then
					If prevFrame < 244 And AnimTime(n\obj)=>244 Then
						PlaySound2(StepSFX(GetStepSound(n\Collider),0,Rand(0,2)),Camera, n\Collider, 8.0, Rnd(0.3,0.5))						
					ElseIf prevFrame < 256 And AnimTime(n\obj)=>256
...
Function MeNPCSeesPlayer%(me.NPCs,disablesoundoncrouch%=False)
	...
	;If me\NPCtype <> NPCtypeD2 Or me\NPCtype <> NPCtypeMTF2 And NoTarget Then Return False
...
;~F#71E#72E#80D#818#88F#8AA#94D#B01#C68#CA0#D4F#D8B#E1F#E9B#FEF#10BE#1175#12CF#12D8#1465
;~F#169C#16C9#16D4#16FC#18E3#1A38#1ABA#1B19#1B99#1BC9#1BEF#1C08#1C88#1D41#1DCB#1DFA#1E0B#1E15#1E9B#1EE0
;~F#1EF9#1F05#1F13#1F36#1F48#1F6C#1F8F#1F9D
;~B#1E4#144C#14E6#1675#1829#1934#1AF5#1B51
;~C#Blitz3D