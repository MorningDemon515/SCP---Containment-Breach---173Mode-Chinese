;SCP - Containment Breach Playable 173 Mod

;    The mod is based on the works of SCP: Containment Breach (scpcbgame.com/).

;    The source code is licensed under the same Creative Commons Attribution-ShareAlike 3.0 License as SCP: Containment Breach.
...
AppTitle "SCP: 收容失效 - 173模组 v"+ModVersion
...
Font6% = AALoadFont("GFX\font\TI-83\TI-83.ttf", 16)

Global CreditsFont%,CreditsFont2%

ConsoleFont% = AALoadFont("GFX\font\cour\Morning Demon.ttf", 16)
...
;player stats -------------------------------------------------------------------------------------------------------
Global KillTimer#, KillAnim%, FallTimer#, DeathTimer#, MoveTimer#
...
Global MTFspawntimer# = 70*600

Global checkpoint# = 70*180
Global checkpoint2# = 70*540
				...
				Case "help"
					...
					Select Lower(StrTemp)
						...
						Case "wireframe"
							...
							CreateConsoleMsg("******************************")
						Case "173j"
							CreateConsoleMsg("HELP - 173j")
							CreateConsoleMsg("******************************")
							CreateConsoleMsg("meow")
							CreateConsoleMsg("******************************")
						Case "spawnitem"
...
;					;[End Block]
				...
				Case "stopsound", "stfu"
                    ...
                    For e.Events = Each Events
                        If e\EventName = "alarm" Then 
                            ...
                            If e\room\NPC[6] <> Null Then RemoveNPC(e\room\NPC[6])
                            Looking = False
				...
				Default
					;[Block]
					CreateConsoleMsg("指令未找到.",255,0,0)
					;[End Block]
			End Select
...
Function CreateDoor.Doors(lvl, x#, y#, z#, angle#, room.Rooms, dopen% = False,  big% = False, keycard% = False, code$="", useCollisionMesh% = False)
	...
	For i% = 0 To 1
		...
		d\interactsprite[i] = CreateSprite()
		Local tex% = LoadTexture_Strict("GFX\interact.png",1+2)
		ScaleSprite d\interactsprite[i],0.05,0.05
		EntityOrder d\interactsprite[i],-1
		EntityTexture d\interactsprite[i],tex
		EntityFX d\interactsprite[i],1+8
	...
	;For i% = 0 To 1
	;	PositionEntity d\interactsprite[i], EntityX(d\buttons[i], True), EntityY(d\buttons[i], True), EntityZ(d\buttons[i], True)
	;Next
	
	HideEntity(d\interactsprite[0])
	HideEntity(d\interactsprite[1])
			...
			If (d\openstate >= 180 Lor d\openstate <= 0) And GrabbedEntity = 0 Then
				For i% = 0 To 1
					If d\buttons[i] <> 0 Then
						...
						If d\buttons[0] <> 0 Then PositionEntity d\interactsprite[0], EntityX(d\buttons[0], True), EntityY(d\buttons[0], True)+0.15, EntityZ(d\buttons[0], True)
						If d\buttons[1] <> 0 Then PositionEntity d\interactsprite[1], EntityX(d\buttons[1], True), EntityY(d\buttons[1], True)+0.15, EntityZ(d\buttons[1], True)
						...
						If Abs(EntityX(Collider)-EntityX(d\buttons[i],True)) < 1.0 Then 
							If Abs(EntityZ(Collider)-EntityZ(d\buttons[i],True)) < 1.0 Then 
								...
								If dist < 0.7 And Looking% = False Then
									Local temp% = CreatePivot()
									
									PositionEntity temp, EntityX(Collider), EntityY(Collider), EntityZ(Collider)
									PointEntity temp,d\buttons[i]
									
									If EntityPick(temp, 290.0) = d\buttons[i] Then
										If ClosestButton = 0 Then
											ClosestButton = d\buttons[i]
											
											If (Not d\locked) And d\KeyCard = False And d\Code = "" Then ;EntityFX d\buttons[i],1+8 : EntityColor d\buttons[i],255-(Float(Sin(MilliSecs()/4.0))*150),255-(Float(Sin(MilliSecs()/4.0))*150),0
												;EntityFX d\buttons[i],1+8 : EntityColor d\buttons[i],255-(Float(Sin(MilliSecs()/4.0))*150),255-(Float(Sin(MilliSecs()/4.0))*150),0
												If ClosestButton = d\buttons[0] Then
													ShowEntity(d\interactsprite[0])
													EntityColor d\interactsprite[0], 255-(Float(Sin(MilliSecs()/4.0))*150),255-(Float(Sin(MilliSecs()/4.0))*150),0
												ElseIf ClosestButton = d\buttons[1] Then
													ShowEntity(d\interactsprite[1])
													EntityColor d\interactsprite[1], 255-(Float(Sin(MilliSecs()/4.0))*150),255-(Float(Sin(MilliSecs()/4.0))*150),0
												EndIf
											EndIf
									...
									FreeEntity temp
								Else
									EntityFX d\buttons[i],0 : EntityColor d\buttons[i],255,255,255
									HideEntity(d\interactsprite[0])
									HideEntity(d\interactsprite[1])
								EndIf							
							EndIf
...
Global CamBaseOBJ%, CamOBJ%, VentAccessOBJ%
...
Function CatchErrors(Location$)
	InitErrorMsgs(6)
	SetErrorMsg(0, "一个错误发生在SCP - 收容失效 v"+ VersionNumber +" 带有SCP-173 模组 v"+ModVersion)
	...
	SetErrorMsg(3, "内存: " + ((TotalVidMem() / 1024) - (AvailVidMem() / 1024)) + " MB/" + (TotalVidMem() / 1024) + " MB" + Chr(10))
	SetErrorMsg(4, "全局内存: " + ((TotalPhys() / 1024) - (AvailPhys() / 1024)) + " MB/" + (TotalPhys() / 1024) + " MB" + Chr(10))
	SetErrorMsg(5, "错误发生在: " + Location + Chr(10) + Chr(10) + "请带着截图来找我们!") 
End Function
...
Repeat
	...
	Else
		...
		If (Not MenuOpen) And (Not InvOpen) And (OtherOpen=Null) And (SelectedDoor = Null) And (ConsoleOpen = False) And (Using294 = False) And (SelectedScreen = Null) And EndingTimer=>0 Then
			...
			checkpoint = checkpoint - FPSfactor
			checkpoint2 = checkpoint2 - FPSfactor
			
			MoveTimer = Max(MoveTimer-FPSfactor,0)
			If MoveTimer =< 0.0 Then
				MoveTimer = 70.0 * 5.0
			Else
				MoveTimer = Max(0.0, MoveTimer - FPSfactor)
			EndIf
			
			Local movesoundtimer = Max(MoveTimer-FPSfactor,0)
			Local ch%, movin% = False
...
;			If movesoundtimer =< 0.0 Then
;;				If (Not movin%) Then
;;					If (ChannelPlaying(ch%)) Then
;;						StopChannel(ch%)
;;					EndIf
;;				Else
;;					ch% = PlaySound_Strict(StoneDragSFX)
;;				EndIf
;				
;				movesoundtimer = 70.0 * 5.0
;			Else
;				movesoundtimer = Max(0.0, movesoundtimer - FPSfactor)
;			EndIf
		...
		Else If SelectedDifficulty\saveType = SAVEONSCREENS And (SelectedScreen<>Null Lor SelectedMonitor<>Null)
			If (Save_MSG<>"Game progress saved." And Save_MSG<>"You cannot save in this location." And Save_MSG<>"You cannot save at this moment.") Or Save_MSG_Timer<=0
...
Function InitCredits()
	...
	CreditsFont% = LoadFont_Strict("GFX\font\cour\Morning Demon.ttf", 21)
	CreditsFont2% = LoadFont_Strict("GFX\font\courbd\Morning Demon.ttf", 35)
...
Function MovePlayer()
	...
	Local Sprint# = 1.0, Speed# = 0.054 * 5, i%, angle#, de.Decals, d.Doors, n.NPCs
	
	If (Not Looking) Then
		PositionEntity(BodyModel, EntityX(Collider), EntityY(Collider) - 0.32, EntityZ(Collider))
		RotateEntity(BodyModel, 0.0, EntityYaw(Collider), 0.0)
		TurnEntity(BodyModel, 0,180,0)
		UnableToMove% = False
		
		If EnableHeadTurning Then
			If (Not Contained%) Then
				Local ppppvt = CreatePivot()
				PositionEntity ppppvt, EntityX(Collider),(EntityY(Collider)+EntityY(Camera))/2,EntityZ(Collider)
				
				EntityPick(ppppvt, 2.5)
				
				If PickedEntity() <> 0 And EntityClass(PickedEntity()) = "Mesh" And (Not EntityHidden(PickedEntity())) Then ; Only allow 173 to look at meshes
					
					If DebugHUD Then
						EntityFX PickedEntity(),1+8 : EntityColor PickedEntity(),0,0,100-(Float(Sin(MilliSecs2()/4.0))*150)
					Else
						EntityFX PickedEntity(), 0 : EntityColor PickedEntity(), 255,255,255
					EndIf
					
					Local y# = CurveAngle(0, (EntityYaw(Collider)-180)+DeltaYaw(Collider,PickedEntity()), 20.0)
					RotateEntity(HeadModel, 0, y#, 0)
				Else
					RotateEntity(HeadModel, 0, CurveAngle(0, EntityYaw(HeadModel), 20.0), 0)
				EndIf
				
				FreeEntity ppppvt
			EndIf
		Else
			RotateEntity(HeadModel, 0, 0, 0)
		EndIf
	Else
		PositionEntity(BodyModel, EntityX(Collider), EntityY(Collider) - 0.32, EntityZ(Collider))
		UnableToMove% = True
	...
	If SuperMan Then
		...
		If SuperManTimer > 70 * 50 Then
			DeathMSG = "在[DATA REDACTED]中发现的D级连体裤。经过进一步检查，发现连体裤中充满了12.5公斤蓝色灰烬状物质. "
		...
		If CollidedFloor = True Then
			If DropSpeed# < - 0.07 Then 
				If Rnd(100)=1 Then
					...
					de\Size = Rnd(0.08,0.1)*2.0 : EntityAlpha(de\obj, 1.0) : ScaleSprite de\obj, de\Size, de\Size
...
Function MouseLook()
	...
	If KillTimer >= 0 And FallTimer >=0 Then
		...
		;PositionEntity(Camera, EntityX(CamPivot, True), EntityY(CamPivot, True), EntityZ(CamPivot, True))
		;MoveEntity(Camera, 0.0, -0.2, -1.2)
		
		If containtimer > 0.0 Then
			EntityParent Camera, CamPivot
			TurnEntity CamPivot,0,180,0
			Local pick = EntityPick(CamPivot,3.0)
			Local pivot = CreatePivot()
			If PickedEntity() <> 0 Then
				PositionEntity pivot,PickedX(),PickedY(),PickedZ(),True
				PositionEntity Camera,0,0,-Min(Max(EntityDistance(CamPivot,pivot)-0.5,0.0),1.0)
			Else
				PositionEntity Camera,0,0,-1.0
			EndIf
			TurnEntity CamPivot,0,-180,0
			FreeEntity pivot
		Else
		...
		Else
			;mouse_y_speed_1 = mouse_y_speed_1
			;mouse_x_speed_1 = mouse_x_speed_1
		EndIf
		...
		RotateEntity Camera, WrapAngle(user_camera_pitch + Rnd(-CameraShake, CameraShake)), WrapAngle(EntityYaw(CamPivot) + Rnd(-CameraShake, CameraShake)), roll  ;Pitch the user's Camera up And down.
...
Function DrawGUI()
	...
	Local x2#,y2#,z2#
	Local n%, xtemp, ytemp, strtemp$, np.NPCs
	Local r.Rooms
	Local e.Events, it.Items
	
	If MenuOpen Lor ConsoleOpen Lor SelectedDoor <> Null Lor InvOpen Lor OtherOpen<>Null Lor EndingTimer < 0 Lor VentMenuOpen% Then
		ShowPointer()
	Else
		HidePointer()
	EndIf 	
	
	If VentMenuOpen% = True Then ; Vent menu
		For x = 0 To MapWidth
			For y = 0 To MapHeight 
				If (Not MapTemp(x, y) = 0) Then
					DrawRoomFrame((GraphicWidth / 3) + (x * -27), (GraphicHeight / 4) + (y * 27), 30, 30)
					If IsVentRoom(MapName(x, y)) Then
						If MapTemp(x, y) > 0 Then
							For r.Rooms = Each Rooms
								If r\RoomTemplate\Name = MapName(x, y) And PlayerRoom\RoomTemplate\Name <> MapName(x, y) Then
									If r\found% Then
										VentSelectionFrame((GraphicWidth / 3) + (x * -27), (GraphicHeight / 4) + (y * 27), 15, 15, MapName(x,y))
									EndIf
								EndIf
							Next
						EndIf
		...
		Msg = "Right click to close the vent menu."
		MsgTimer = 70 * 8
	EndIf
	...
	If ClosestButton <> 0 And SelectedDoor = Null And InvOpen = False And MenuOpen = False And OtherOpen = Null Then
		If ClosestDoor\Code <> "" Or ClosestDoor\KeyCard <> 0 Then ; identifiers for doors
		Else
			If MouseUp1 Then
				MouseUp1 = False
	...
	If HUDenabled Then 
		...
		If DebugHUD Then
			...
			AAText x - 50, 450, "ending timer: " + containtimer
			If Curr106 <> Null
...
Function DrawMenu()
	...
	If MenuOpen Then
		...
		Else
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "NEUTRALIZED",False,True)
			AASetFont Font1
		End If		
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			...
			Select OptionsMenu
				...
				Case 3 ;Controls
					...
					InputBox(x + 200 * MenuScale, y + 100 * MenuScale,100*MenuScale,20*MenuScale,KeyName(Min(KEY_SAVE,210)),11)	
					AAText(x, y + 120 * MenuScale, "Open/Close Console")
					InputBox(x + 200 * MenuScale, y + 120 * MenuScale,100*MenuScale,20*MenuScale,KeyName(Min(KEY_CONSOLE,210)),12)
...
;					
;					Color 255,255,255
;					AAText(x, y, "Enable SCP-173 head turning:")
;					EnableHeadTurning% = DrawTick(x + 270 * MenuScale, y, EnableHeadTurning%)
;					If MouseOn(x+270*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
...
Function LoadEntities()
	...
	;If Enable3DMenu% Then NullMenu3D()
	...
	BodyModel = LoadMesh_Strict("GFX\npcs\173body.b3d")
	ScaleEntity BodyModel, 0.054,0.054,0.054
	PositionEntity BodyModel, 0, 0.5, -1.5, True
	EntityParent BodyModel, Collider
	;HideEntity BodyModel
	
	HeadModel = LoadMesh_Strict("GFX\npcs\173Head.b3d")
	ScaleEntity HeadModel, 0.054,0.054,0.054
	PositionEntity HeadModel, 0.0, 0.5, -1.5, True
	EntityParent (HeadModel, BodyModel)
	;HideEntity HeadModel
	
	ContainmentBox = LoadMesh_Strict("GFX\173box.b3d")
	ScaleEntity ContainmentBox, RoomScale, RoomScale, RoomScale
	PositionEntity ContainmentBox, 0,0.45,-1.5, True
	EntityParent (ContainmentBox, BodyModel)
	...
	HandIcon% = LoadImage_Strict("GFX\interact.png")
	HandIcon2% = HandIcon
	...
	ButtonTexture = LoadTexture_Strict("GFX\map\Props\KeyPad.jpg")
	ButtontextureLocked = LoadTexture_Strict("GFX\map\Props\KeyPadLocked.jpg")
	...
	PositionEntity(CamPivot, EntityX(Collider, True), EntityY(Collider, True) + 1.0 - 0.2, EntityZ(Collider, True) + 0.10)
	EntityParent(CamPivot, Collider)
...
;	HideEntity OBJTunnel(6)
	...
	ButtonOBJ = LoadMesh_Strict("GFX\map\Props\Button.b3d")
	HideEntity ButtonOBJ
	ButtonKeyOBJ = LoadMesh_Strict("GFX\map\Props\ButtonKeycard.b3d")
	HideEntity ButtonKeyOBJ
	ButtonCodeOBJ = LoadMesh_Strict("GFX\map\Props\ButtonCode.b3d")
	HideEntity ButtonCodeOBJ	
	ButtonScannerOBJ = LoadMesh_Strict("GFX\map\Props\ButtonScanner.b3d")
	...
	VentAccessOBJ = LoadMesh_Strict("GFX\map\Props\ventilation_access.b3d")
	HideEntity(VentAccessOBJ)
...
Function InitNewGame(mode%=0)
	CatchErrors("Uncaught (InitNewGame)")
	Local i%, de.Decals, d.Doors, it.Items, r.Rooms, sc.SecurityCams, e.Events
	
	DrawLoading(45)
	
	NullMenu3D() ; this is already run in the fuckin drawloading function but whatev
	...
	For r.Rooms = Each Rooms
		...
		If (r\RoomTemplate\Name = "start" And IntroEnabled = False) Then 
			PositionEntity (Collider, EntityX(r\obj)+32*RoomScale, EntityY(r\obj)+215*RoomScale, EntityZ(r\obj)+1000*RoomScale);1072*RoomScale)
			PlayerRoom = r
		ElseIf (r\RoomTemplate\Name = "173" And IntroEnabled) Then
			PositionEntity (Collider, EntityX(r\obj)+ 1472.0 *RoomScale, 100.0*RoomScale, EntityZ(r\obj)+ 912.0 *RoomScale)
			PlayerRoom = r
		EndIf
	Next
...
Function NullGame(playbuttonsfx%=True)
	...
	Local wp.WayPoints, twp.TempWayPoints, r.Rooms, it.Items, v.Vents, b.BreakableObjects, c.Cutscenes
	...
	breaktestroom2glass% = False
	playclosetevent% = False
	...
	For c.Cutscenes = Each Cutscenes
		Delete c
	Next
...
Function UpdateMTF%()
	...
	MTFspawntimer = MTFspawntimer - FPSfactor
		...
		If Rand(30)=1 And PlayerRoom\RoomTemplate\Name$ <> "dimension1499" Then
			...
			If entrance <> Null Then
				If MTFspawntimer <= 0 Then
					...
					Local leader.NPCs
					For i = 0 To 2
						n.NPCs = CreateNPC(NPCtypeMTF2, EntityX(entrance\obj)+0.3*(i-1), 1.0,EntityZ(entrance\obj)+8.0)
...
Function RenderWorld2()
	...
	For np.NPCs = Each NPCs
		...
		np\NVY = EntityY(np\Collider,True) + 1
	...
	For np.NPCs = Each NPCs
		If np\NVName <> "" Then 
			...
			Local Dist# = EntityDistance(np\Collider, Collider)
			
			If Dist3# < 21 And np\IsDead <> True And np\IsLooking = True Then
				
				UpdateNPCBlink(np)
				
				CanSave% = False
				Local Temp%, x%, y%, z%, i%, yawvalue#, pitchvalue#
				yawvalue# = -DeltaYaw(Camera, np\Collider)
				If yawvalue > 90 And yawvalue <= 180 Then yawvalue = 90
				If yawvalue > 180 And yawvalue < 270 Then yawvalue = 270
				pitchvalue# = -DeltaPitch(Camera, np\Collider)
				If pitchvalue > 90 And pitchvalue <= 180 Then pitchvalue = 90
				If pitchvalue > 180 And pitchvalue < 270 Then pitchvalue = 270
				
				If HUDenabled Then
					Color 100,100,100
					Local rndoffset = Rnd(1,4)
					RenderSmoothBar(GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 32, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32, 90, 1, np\BlinkTimer#/7)
					;DrawImage BlinkIcon, viewport_center_x + (xValue * viewport_center_x) - 40, viewport_center_y - (yValue * viewport_center_y) - 2
					If DebugHUD Then
						Color 0,255,0
						AAText GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 32, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32,np\Reload,True,True
					Else
					EndIf
					Color 100,100,100
					If np\BlinkTimer > 70*4 Then
						Rect(GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 75 - 1, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32 - 1, 30 + 2, 30 + 2, False)
						DrawImage BlinkIcon, GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 75, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32
					ElseIf np\BlinkTimer < 0.0 Then
						Rect(GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 75 - 1, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32 - 1, 30 + 2, 30 + 2, False)
						DrawImage BlinkIcon2, GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 75, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32
					Else
						Color 255,0,0
						Rect(GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 75 - 1 + rndoffset, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32 - 1 + rndoffset, 30 + 2, 30 + 2, False)
						Color 100,100,100
						DrawImage BlinkIcon, GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3) - 75 + rndoffset, GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 32 + rndoffset
					EndIf
				Else
				EndIf
			Else
